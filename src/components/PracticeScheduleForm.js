...
const PracticeScheduleForm = ({ onClose, isSubmitted }) => {
  ...
  const [selectedCoaches, setSelectedCoaches] = useState([
    { coachId: user.coachId, name: user.username }
  ]);
    ...
    switch (name) {
      ...
      case "startTime":
        if (!value) {
          ...
          new Date(`1970-01-01T${value}:00`) >=
            new Date(`1970-01-01T${formData.endTime}:00`)
        ) {
      ...
      case "endTime":
        if (!value) {
          ...
          formData.startTime &&
          new Date(`1970-01-01T${value}:00`) <=
            new Date(`1970-01-01T${formData.startTime}:00`)
        ) {
      ...
      <div className=" flex items-center justify-center">
        <div
          className={` bg-white  ${uploading
            ? "opacity-80"
            : "bg-opacity-100"} m-5 md:m-0 p-8 rounded-3xl shadow-lg max-w-xl w-full relative`}
        >
          ...
          >
            ...
            <div className="md:col-span-2 col-span-1">
              ...
              <div
                tabIndex={-1}
                className="flex text-gray-600 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-[#00175f]"
              >
                <input
                  type="text"
                  ...
                  value={
                    selectedCoaches &&
                    selectedCoaches.map(coach => coach && coach.name).join(", ")
                  }
