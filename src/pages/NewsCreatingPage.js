...
const [searchAuthor, setSearchAuthor] = useState(""); // State for author search
  ...
  const filteredNews = createdNews.filter((news) => {
    const searchTerm = searchHeading.toLowerCase();
    
    const matchesHeading = news.heading.toLowerCase().includes(searchTerm);
    const matchesAuthor = news.author && news.author.toLowerCase().includes(searchTerm);
...
<div className="flex items-center space-x-2">
  <div className="relative inline-block">
    <FontAwesomeIcon
      ...
      onClick={() => document.getElementById('datepicker-trigger').focus()} // Focus the DatePicker input
    />
    <DatePicker
      ...
      className="text-gray-600 border-0 focus:outline-none" // No border and no outline on focus
      placeholderText="Select a date"
      calendarClassName="z-50" // This ensures the calendar is displayed above other elements
    />
  </div>
  ...
  {searchDate && (
    <button
      onClick={() => setSelectedDate(null)} // Reset searchDate
