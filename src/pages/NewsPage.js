...
const NewsPage = () => {
  ...
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentNews = filteredNews.slice(indexOfFirstItem, indexOfLastItem); // Use filteredNews here
                    ...
                    <div key={news.id} className="mb-4 sm:mb-6">
                      <div className="flex flex-col sm:flex-row mb-4">
                        ...
                        <div className="flex-1">
                          ...
                          <span className="text-xxs sm:text-xs text-gray-500 mt-2 block">
                            {formatDate(news.createdOn)} • {timeAgo(news.createdOn)} • {news.author}
                          </span>
                        </div>
                      </div>
                      {index < currentNews.length - 1 && <hr className="border-gray-300 my-4" />}
