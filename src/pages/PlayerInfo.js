...
const paginatedData = sortedPlayerData.slice(
  ...
  // Calculate total pages based on filteredPlayers length
const totalPages = Math.ceil(filteredPlayers.length / rowsPerPage);

// Sort filtered players by status before slicing for pagination
const sortedFilteredData = [...filteredPlayers].sort((a, b) => {
  if (a.status === "Active" && b.status !== "Active") return -1;
  if (a.status !== "Active" && b.status === "Active") return 1;
  return 0;
});
